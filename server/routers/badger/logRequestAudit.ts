...
// Insert logs in batches inside a single transaction, so a failure partway
// through leaves no rows behind (the caller's retry path re-queues the whole
// slice, which would otherwise duplicate rows that were already committed).
//
...
// fires before any insert has executed - the inserts then run afterwards in
// autocommit mode, one implicit transaction each, and the surrounding
// try/catch cannot see their rejections either. Driving them synchronously is
// what makes the transaction real on SQLite. node-postgres is genuinely async
// and keeps the awaited form.
async function insertAuditLogsAtomically(
    logsToWrite: (typeof requestAuditLog.$inferInsert)[]
): Promise<void> {
    // Batch insert logs in groups of 25 to avoid overwhelming the database
    const BATCH_DB_SIZE = 25;

    if (driver === "sqlite") {
        // Both branches are cast because only one driver's types exist per
        // build - server/db/index.ts is generated by `npm run set:sqlite` /
        // `set:pg`, so whichever branch is inactive cannot typecheck against
        // the active driver's signature (under the pg build this branch fails
        // with "Property 'run' does not exist on type 'PgInsertBase'", and
        // under sqlite the pg branch's async callback is likewise rejected).
        (logsDb as any).transaction((tx: any) => {
            for (let i = 0; i < logsToWrite.length; i += BATCH_DB_SIZE) {
                tx.insert(requestAuditLog)
                    .values(logsToWrite.slice(i, i + BATCH_DB_SIZE))
                    .run();
            }
        });
        return;
    ...
    await (logsDb as any).transaction(async (tx: any) => {
        for (let i = 0; i < logsToWrite.length; i += BATCH_DB_SIZE) {
            await tx
...
async function flushAuditLogs() {
    ...
    try {
        ...
        await insertAuditLogsAtomically(logsToWrite);
